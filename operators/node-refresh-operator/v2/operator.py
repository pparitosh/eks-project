...
def refresh_nodes(spec, status, name, namespace, logger, **kwargs):
    ...
    if not matching_nodes:
        update_status(custom_api, namespace, name, 'Completed', 'No nodes found', [])
        return {'phase': 'Completed', 'message': 'No nodes found'}
    ...
    if not pending_nodes:
        update_status(custom_api, namespace, name, 'Completed', 
                     'All nodes refreshed', completed_nodes)
        return {'phase': 'Completed', 'message': 'All nodes refreshed'}
    ...
    update_status(custom_api, namespace, name, 'Processing', 
    ...
    try:
        ...
        update_status(custom_api, namespace, name, 'Failed', 
...
def update_status(custom_api, namespace, name, phase, message, completed_nodes):
